...
import 'package:root2route/features/auctions/ui/auction_checkout_screen.dart';
...
class _BuyerAuctionsScreenState extends State<BuyerAuctionsScreen>
  ...
  Future<void> _fetchParticipated() async {
    ...
    try {
      ...
      if (res['success'] == true) {
        ...
        final enrichedList = await _enrichAuctionsWithImages(rawList);

        if (!mounted) return;
        setState(() {
          _participated = enrichedList;
  ...
  Future<List<dynamic>> _enrichAuctionsWithImages(List<dynamic> auctions) async {
    return await Future.wait(auctions.map((auction) async {
      if (auction is Map<String, dynamic> || auction is Map) {
        final Map<String, dynamic> updatedAuction = Map<String, dynamic>.from(auction);
        final productId = updatedAuction['productId'] ?? updatedAuction['ProductId'];
        
        if (productId != null && productId.toString().isNotEmpty) {
          try {
            final productRes = await _api.getProductById(productId.toString());
            if (productRes['success'] == true && productRes['data'] != null) {
              final pData = productRes['data'];
              final imageUrl = pData['mainImageUrl'] ?? pData['MainImageUrl'] ?? pData['image'] ?? pData['imageUrl'];
              if (imageUrl != null) {
                updatedAuction['productImageUrl'] = imageUrl;
              }
            }
          } catch (e) {
            debugPrint('[Enrich Image] Failed to fetch product $productId: $e');
          }
        }
  ...
  Future<void> _handleCheckout(String auctionId, String title) async {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder:
            (_) => AuctionCheckoutScreen(auctionId: auctionId, title: title),
      ),
    );
  }
...
class _ParticipatedAuctionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final auctionId =
        (auction['id'] ?? auction['Id'] ?? auction['auctionId'] ?? '')
            .toString();
    ...
    debugPrint(
      '[BuyerAuctions] auction keys: ${(auction as Map).keys.toList()}  | currentHighestBid=${auction['currentHighestBid']}',
    );
...
class _WonAuctionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final title = _resolveTitle(auction);
    final displayUrl = _resolveImageUrl(auction);
    final rawPrice = auction['currentHighestBid'] ?? auction['finalPrice'] ?? 0;
